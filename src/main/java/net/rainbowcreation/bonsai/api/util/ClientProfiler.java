...
public class ClientProfiler {
    ...
    public static final boolean ENABLED = Config.PROFILER_ENABLED;
    private static final String OUTPUT_FILE = Config.PROFILER_OUTPUT_FILE;
    private static final int SAMPLE_RATE = Config.PROFILER_SAMPLE_RATE;

    private static final Map<Integer, RequestTiming> timings = new ConcurrentHashMap<>();
    ...
    static {
        ...
        if (ENABLED) {
            ...
            Runtime.getRuntime().addShutdownHook(
                ThreadUtil.newDaemonThread(ClientProfiler::printStats, "client-profiler-shutdown")
            );
        }
    }
     ...
     */
    public static void onSend(int reqId) {
        if (!ENABLED) return;
     ...
     */
    public static void onFlush(int reqId) {
        if (!ENABLED) return;
     ...
     */
    public static void onResponse(int reqId) {
        if (!ENABLED) return;
